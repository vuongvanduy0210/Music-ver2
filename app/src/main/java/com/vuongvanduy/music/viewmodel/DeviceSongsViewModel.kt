...
import com.vuongvanduy.music.util.DEVICE_SONGS_FRAGMENT_TAG
...
class DeviceSongsViewModel : ViewModel() {
    ...
    private fun isExistInList(list: MutableList<Song>, song: Song): Boolean {
        if (list.isEmpty()) {
            return true
        }
        for (i in 0 until list.size) {
            if (song.getName() == list.get(i).getName() &&
                song.getSinger() == list.get(i).getSinger()
            ) {
                return false
            }
        }
